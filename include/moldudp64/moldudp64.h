...
class MoldUDP64Packet 
{
   public:
    ...
    bool add_message(const void* data, std::uint16_t size);                                 

    static MoldUDP64Packet heartbeat(const std::string& session,
                                     SeqNo sequence_number);

    static MoldUDP64Packet end_of_session(const std::string& session,
                                          SeqNo sequence_number);
